...
class UsersView(FlaskView, UIView):
    ...
    def disable(self, id):
        ...
        if deletion_date > datetime.now():
            ...
            flash(
                f'User was disabled and will be automatically deleted on {deletion_date.strftime("%d %B %Y")}',
                "danger",
            )
        else:
            flash(
                "User was disabled and will be automatically deleted within one hour",
                "danger",
            )
